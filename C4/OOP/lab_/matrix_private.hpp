...
void Matrix<T>::_checkSizes(const Matrix<T> &matrix) const 
{
    if (_rows != matrix._rows || _cols != matrix._cols) {
        ...
        throw InvalidShapeError(asctime(curtime), __FILE__, __LINE__, "Different matrix sizes");
    }
}

template <typename T>
SharedPtr<typename Matrix<T>::MatrixRow[]> Matrix<T>::_allocMemory(size_t rows, size_t cols)
{
    ...
    try {
        ...
        time_t cur_time = time(NULL);
        auto curtime = localtime(&cur_time);
        throw MemoryAllocationError(asctime(curtime), __FILE__, __LINE__, "_allocateMemory function error");
    }
...
void Matrix<T>::_checkMultSizes(const Matrix<T> &matrix) const 
{
    if (_cols != matrix._rows) {
        time_t cur_time = time(NULL);
        auto curtime = localtime(&cur_time);
        throw InvalidShapeError(asctime(curtime), __FILE__, __LINE__,
