...
{
    auto tmp = _allocMemory(_cols, _rows);
...
void Matrix<T>::inverse() 
{
    ...
    if (!isSquare() || !det) {
        ...
        throw InvalidShapeError(asctime(curtime), __FILE__, __LINE__, "Only square matrix can be inversed and determinant should be > 0");
    }
...
T Matrix<T>::determinant() const
{
    if (!isSquare()) {
        time_t cur_time = time(NULL);
        auto curtime = localtime(&cur_time);
        throw InvalidShapeError(asctime(curtime), __FILE__, __LINE__, "Matrix should be square to get determinant;");
    }
...
static void _excludeCopy(Matrix<T> &target, const Matrix<T> &source, size_t ex_row, size_t ex_col) 
{
    if (target.GetRows() != source.GetRows() - 1 || target.GetColumns() != source.GetColumns() - 1) 
    {
        time_t cur_time = time(NULL);
        auto curtime = localtime(&cur_time);
        throw InvalidShapeError(asctime(curtime), __FILE__, __LINE__, "Target has wrong size");
    }
