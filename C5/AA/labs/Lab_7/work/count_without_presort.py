...
test_sizes = [100000, 150000, 200000, 250000, 300000, 350000, 400000, 450000, 500000]
...
for record in test_records:
    ...
    for i in range(0, iters):
        res = seq_search(record, key_start)
    #res /= iters
    t_seq_start.append(res)
    ...
    #res /= iters
    t_bin_start.append(res)

    res = 0
    for i in range(0, iters):
        new_record = gen_dict_for_seg(record)
        res = seg_search(new_record, key_start)
    #res /= iters
    t_seg_start.append(res)


    res = 0
    for i in range(0, iters):
        res = seq_search(record, key_center)
    #res /= iters
    t_seq_center.append(res)

    res = 0
    for i in range(0, iters):
        list_keys = sorted(list(record.keys()))
        res = bin_search_presort(record, key_center, list_keys)
    #res /= iters
    t_bin_center.append(res)

    res = 0
    for i in range(0, iters):
        new_record = gen_dict_for_seg(record)
        res = seg_search(new_record, key_center)
    #res /= iters
    t_seg_center.append(res)


    res = 0
    for i in range(0, iters):
        res = seq_search(record, key_end)
    #res /= iters
    t_seq_end.append(res)

    res = 0
    for i in range(0, iters):
        list_keys = sorted(list(record.keys()))
        res = bin_search_presort(record, key_end, list_keys)
    #res /= iters
    t_bin_end.append(res)

    res = 0
    for i in range(0, iters):
        new_record = gen_dict_for_seg(record)
        res = seg_search(new_record, key_end)
    #res /= iters
    t_seg_end.append(res)


    res = 0
    for i in range(0, iters):
        res = seq_search(record, key_none)
    #res /= iters
    t_seq_none.append(res)

    res = 0
    for i in range(0, iters):
        list_keys = sorted(list(record.keys()))
        res = bin_search_presort(record, key_none, list_keys)
    #res /= iters
    t_bin_none.append(res)

    res = 0
    for i in range(0, iters):
        new_record = gen_dict_for_seg(record)
        res = seg_search(new_record, key_none)
    #res /= iters
    t_seg_none.append(res)
